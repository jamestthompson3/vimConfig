" improvedsearch.vim - display current & total number of search matches
...
if exists('g:loaded_improvedsearch')
  finish
endif
let g:loaded_improvedsearch = 1

" Setup options.
let g:improvedsearch_line_limit=1000000
let g:improvedsearch_improved_star=1
let g:improvedsearch_star_case=1
...
command! -bar Improvedsearch call <SID>PrintMatches()

" If user has mapped 'g/', don't override it.
silent! nmap <unique> g/ <Plug>improvedsearch

noremap  <Plug>improvedsearch <Nop>
noremap! <Plug>improvedsearch <Nop>
nnoremap <silent> <Plug>improvedsearch :call <SID>PrintMatches()<CR>

" Remap search commands (only if they're not mapped by the user).
silent! nmap <silent><unique> n n<Plug>improvedsearch
silent! nmap <silent><unique> N N<Plug>improvedsearch

silent! map <unique> *  <Plug>ImprovedStar_*<Plug>improvedsearch
silent! map <unique> #  <Plug>ImprovedStar_#<Plug>improvedsearch
silent! map <unique> g* <Plug>ImprovedStar_g*<Plug>improvedsearch
silent! map <unique> g# <Plug>ImprovedStar_g#<Plug>improvedsearch
...
silent! cmap <unique><expr> <CR>
    \ "\<CR>" . (getcmdtype() =~ '[/?]' ? "<Plug>improvedsearch" : "")

if exists('*getcmdwintype')
  " getcmdwintype() requires Vim 7.4.392. If it's not available, disable
  " support for command window searches (q/, q?).
  augroup improvedsearch_cmdwin
    autocmd!
    autocmd CmdWinEnter *
      \ if getcmdwintype() =~ '[/?]' |
      \   silent! nmap <buffer><unique> <CR> <CR><Plug>improvedsearch|
      \ endif
  augroup END
endif
...
function! s:StarSearch(cmd)
  if !g:improvedsearch_improved_star | return a:cmd | endif
  ...
  let case_char = (g:improvedsearch_star_case ? '\C' : '\c')
...
function! s:PrintMatches()
  ...
  if line('$') > g:improvedsearch_line_limit
    ...
    let [l:current, l:total] = improvedsearch#MatchCounts()
...
function! improvedsearch#MatchCounts()
  ...
  if exists('b:improvedsearch_cache_key') && b:improvedsearch_cache_key ==# cache_key
    let before = s:MatchesAbove(b:improvedsearch_cache_val)
    let total = b:improvedsearch_cache_val[-1]
  else
    let before = (line('.') == 1 ? 0 : s:MatchesInRange('1,-1'))
    let total = before + s:MatchesInRange(',$')
  endif

  let b:improvedsearch_cache_val = [line('.'), before, total]
  let b:improvedsearch_cache_key = cache_key
...
"   only add <Plug>improvedsearch after search, not ex command or anything else.
