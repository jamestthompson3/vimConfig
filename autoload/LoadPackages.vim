...
let s:full_path = join([$HOME,'.cache', 'vimfiles', 'repos', 'github.com', 'Shougo', 'dein.vim'], s:file_separator)
let s:state_path = join([$HOME,'vim','bundles'], s:file_separator)
exec 'set runtimepath+='.fnameescape(s:full_path)
 if dein#load_state(s:state_path)
